...
#ifdef USE_ORIGIN_LIFTPROJECTIVE
//origin
void
...
//    printf("liftProjective from orbslam3");
    cv::Point2f pw(m_inv_K11 * p(0) + m_inv_K13, m_inv_K22 * p(1) + m_inv_K23);
    float scale = 1.f;
    ...
    if (theta_d > 1e-8) {
        ...
        for (int j = 0; j < 10; j++) {
            float theta2 = theta * theta, theta4 = theta2 * theta2, theta6 = theta4 * theta2, theta8 =
                    theta4 * theta4;
            float k0_theta2 = mParameters.k2() * theta2, k1_theta4 = mParameters.k3() * theta4;
            float k2_theta6 = mParameters.k4() * theta6, k3_theta8 = mParameters.k5() * theta8;
...
//    originP(0) = sin(theta) * cos(phi);
//    originP(1) = sin(theta) * sin(phi);
//    originP(2) = cos(theta);
//
//    Eigen::Vector2d p1 = Eigen::Vector2d(P.x() / P.z(), P.y() / P.z());
//    Eigen::Vector2d p2 = Eigen::Vector2d(originP.x() / originP.z(), originP.y() / originP.z());
//
//    std::cerr << "p1-p2: " << (p1-p2).transpose() << std::endl;
}
#endif
